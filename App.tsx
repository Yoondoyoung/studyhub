...
export default function App() {
  ...
  const checkSession = async () => {
    ...
    if (savedToken) {
      try {
        const response = await fetch(`https://${projectId}.supabase.co/functions/v1/server/auth/session`, {
  ...
  const handleLogin = async (email: string, password: string) => {
    try {
      const response = await fetch(`https://${projectId}.supabase.co/functions/v1/server/auth/signin`, {
        method: 'POST',
    ...
    <div className="min-h-screen bg-[#f5f5f7] flex">
      ...
      <div className="flex-1 ml-56">
        ...
        <main className="p-6">
          ...
          {currentPage === 'friends' && (
            <FriendsPage accessToken={accessToken} projectId={projectId} />
          )}
          {currentPage === 'settings' && (
            <SettingsPage accessToken={accessToken} projectId={projectId} />
          )}
        </main>
      </div>
